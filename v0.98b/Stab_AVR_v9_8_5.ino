...
//--исправлена ошибка записи нулевого номинала мощности
//--
//-------------------------
#include <Wire.h>
#include <EEPROM.h>
...
//
#define U_LINE 230L   		// Номинальное значение действующего напряжения в сети, для которого указана номинальная мощность ТЭНа
      ...
      switch (cnt_dspMenu) { //=====Проверяем режимы меню
        case DSP::Pnom_MENU:  {  //=============Если мы в начальном меню выбора номинальной мощности, то...
          if (fl.dspTimeout) {                  // Если кнопки слишком долго не нажимались...
              uint16_t tmp_Pnom_arr = Pnom_arr[0];
              if (tmp_Pnom_arr && (tmp_Pnom_arr < 10000)) {	// и есть корректное записанное значение, уходим
              Pnom = tmp_Pnom_arr; 				// По умолчанию установим номинальную мощность из нулевой ячейки
              cnt_Pnom_number = 0;              //
