...
#define pin_RAZGON_OFF A3 //  Пин входа отключения разгона
#define pin_STAB_OFF A2   //  Пин входа отключения стабилизатора (пока не задействован)
//
//===========вЫходные выводы===========
...
#define TURN_SSR_ON PORTD |=  (1 << pin_TOut) // Включаем ТЭН
#define TURN_SSR_OFF PORTD &=  ~(1 << pin_TOut) // Выключаем ТЭН
...
#define TURN_RELAY_ON PORTD |=  (1 << pin_TRelay) // Включаем ТЭН
#define TURN_RELAY_OFF PORTD &=  ~(1 << pin_TRelay) // Выключаем ТЭН
#else // управление контактным реле низким уровнем
#define TURN_RELAY_ON PORTD &=  ~(1 << pin_TRelay) // Выключаем ТЭН
#define TURN_RELAY_OFF PORTD |=  (1 << pin_TRelay) // Включаем ТЭН
#endif
//
#define pin_LED 5
#define PORT_LED PORTB
#define TURN_LED_ON  PORT_LED |=  (1 << pin_LED)  // Включаем светодиод (5 пин порта B для NANO)
#define TURN_LED_OFF PORT_LED &=  ~(1 << pin_LED) // Выключаем светодиод (5 пин порта B для NANO)
//
...
const uint8_t array_size = max(Pnom_ARRAY_SIZE,PDMset_SIZE);
...
//
// В EEPROM хранятся значения номинальных мощностей ТЭНа (каждая занимает 2 байта, количество определяется величиной Pnom_ARRAY_SIZE)
...
#define SRVDATA_ARRAY_SIZE 14             // Размер блока памяти для вспомогательных данных
...
//
void set_Pust(void) { //===========Подпрограммка подсчета Pust================
  Pust = (uint16_t)calc_PDMtoPust(PDMust);
} //===========Подпрограммка подсчета Pust(конец)================
//
uint32_t calc_PDMtoPust(const uint16_t pdm) { //=========Функция пересчета PDM в Pust================
  uint32_t p;
  p = Pnom * 2; p *= (long)pdm; p /= CICLE;  p = p; p++; p /= 2; // Считаем уставку с округлением
  return p;
} //=========Функция пересчета PDM в Pust(конец)================
//
...
//
  uint8_t idx;
  uint16_t value;
  for (idx = 0; idx < Pnom_ARRAY_SIZE; idx++) {
...
//
// tmpcount += get_noempty_cells(PDMtmp, tmpcount, start_cell, end_cell, (PDMset_SIZE-2));
//
...
//
  ...
  //
  value = CICLE / PDMset_SIZE;
  for (uint8_t idx=0; idx < PDMset_SIZE - 1; idx++) {
    PDMset[0][idx] = (long)value * (idx + 1);
  }
  PDMset[0][PDMset_SIZE - 1] = CICLE;
  //
  ////===Определим границы области для записи уставок
  //
  addr = (EEPROM.length() - SRVDATA_ARRAY_SIZE - 2 * Pnom_ARRAY_SIZE)/(Pnom_ARRAY_SIZE);  // Размер области для записи уставок в EEPROM
  if (addr & 1) addr--; // Округлим до четного вниз, чтобы влезало целое количество двухбайтовых слов
  start_addr = Pnom_ARRAY_SIZE * 2 + cnt_Pnom_number * addr;  // Начальный адрес области
  ...
  //if (value != EMPTY_CELL_VALUE) {          // Если считанное значение не пустое, то читаем из конца области
  if ((value <= CICLE) && value) {      // Если считанное значение корректное, то читаем из конца области
    read_region_end_addr = start_addr + 2 * ((PDMset_SIZE - 2) - 1);  // На 3 меньше, потому что одно значение уже есть, как минимум, другое в память не пишется, ну и размер на единицу больше наибольшего индекса, Так-то!
    tmpcount = get_noempty_cells(PDMtmp, 0, (end_addr - 2 * (PDMset_SIZE-2)), end_addr, (PDMset_SIZE-2));
  }
  if (tmpcount < PDMset_SIZE - 1) {         // Проверим массив уставок на заполненность и если не полон, то читаем дальше
    tmpcount += get_noempty_cells(PDMtmp, tmpcount, start_addr, read_region_end_addr, (PDMset_SIZE-2));
  }
  //
  //---Прочитали---
  //
  if (tmpcount) { // Если записанные уставки есть, то...
    ...
    { // Уберем дублирующиеся значения
      ...
      for (uint8_t i = 0; i < PDMset_SIZE; i++) {   // Пробежимся по обоим массивам
        for (uint8_t j = 0; j <= tmpcount; j++) {
          if (PDMset[0][i] == PDMtmp[0][j]) PDMdiff[i] = 1; // И заполним массив совпадений
        }
      }
      for (uint8_t i = 0; i < PDMset_SIZE - 1; i++) { // Сортируем
        for (uint8_t j = i + 1; j < PDMset_SIZE; j++) {
...
void Pins_init(void) { //======Инициализация входов/выходов контроллера=========
  ...
  digitalWrite(pin_OLED_res, LOW);    // Сбрасываем дисплей (!!! НЕ ЗАБЫТЬ ПЕРЕКЛЮЧИТЬ НА ВЫСОКИЙ !!!) TODO!
#endif
  pinMode(pin_RAZGON_OFF, INPUT_PULLUP);  // Определяем вывод отключения разгона, как вход и подтягиваем его внутренним резюком к VCC
  pinMode(pin_STAB_OFF, INPUT_PULLUP);    // Определяем вывод отключения стаба, как вход и подтягиваем его внутренним резюком к VCC
...
void PDM_(void) { //===========Подпрограмма управления твердотельным реле ТЭНа================
  ...
  if (fl.PP_tm) {
    //ps -= fl.Tout; // Декрементируем ps, если в прошлую отрицательную полуволну твердотельное реле было включено
    if (fl.Tout) ps--;
  }
  else {
    //ps += fl.Tout;           // Инкрементируем ps, если в прошлую положительную полуволну твердотельное реле было включено
    if (fl.Tout) ps++;
  }
//
...
void Buttons_(void) { //==============Опрос кнопок=====================
  ...
  if (bt.clear_old) { //=====Стираем старую уставку, если нужно
    ...
    //EEPROM.put(clear_old_addr,(uint16_t)EMPTY_CELL_VALUE);  // Стираем самую старую уставку
    bt.clear_old = 0;                                     // Снимаем флажок стирания
  }
  fl.razg_off = !digitalRead(pin_RAZGON_OFF); // Прочитаем состояние вывода отключения разгона
  if (fl.razg_off) {  // Если есть внешний сигнал останова разгона...
  ...
  if ( (butt_count == DEBOUNCE) || fl.dspTimeout ) {  // Есть нажатая кнопка или достаточная пауза после нажатия или таймаут выхода из меню
    if (butt) {  // Если нажата кнопка, сбросим таймер ожидания выхода из меню
      cnt_menuWDT = 0;
    }
      ...
      case 1:  {  //=============Если мы в меню выбора уставки, то...
        ...
        switch (butt) {
          case 1: { //=====По кнопке "Р-" перебираем значения
            if (++cnt_PDMcount > PDMset_SIZE - 1) cnt_PDMcount=0; //Перебираем значения уставок мощности ТЭНа
  //          fl.butt = 0;            //После нажатия должна быть пауза
            break;                    //Закончили
          }
          case 2: { //=====По кнопке "Р+" перебираем значения
            if (cnt_PDMcount-- == 0) cnt_PDMcount=PDMset_SIZE - 1;//Перебираем значения уставок мощности ТЭНа
  //          fl.butt = 0;            //После нажатия должна быть пауза
            break;                    //Закончили
          }
          case 4: { //=====По кнопке "стоп" записываем уставку, если нужно, принимаем и выходим
            ...
            if (fl.writable) {  // Если уставки запоминаются...
              if (!PDMset[1][cnt_PDMcount]) { // Если просят записать НЕ уже записанное...
                ...
                if (cnt_PDMcount == PDMset_SIZE - 1) {  // Если новое значение - последнее в списке
                  //
                  if (!old_addr) {  // Если в массиве уставок есть незаписанные в EEPROM значения, то сначала стираем их
                    boolean swapped = 1; uint8_t upper_index = PDMset_SIZE - 1; //=====Пузырьковая сортировка
                  ...
                  for (uint8_t x = 0; x < PDMset_SIZE - 1; x++) { // Сдвинем массив
          ...
          case 4:
            if (PDMust == 0) {    //Если мы не в меню и мощность ТЭНа нулевая, то...
              ...
              fl.dspRefresh = 1;  //Ставим флаг обновления экрана
                ...
                boolean isnew = 1;
                for (int8_t x = PDMset_SIZE - 1; x >= 0; x--) {  // Проверим новое значение на совпадение с уже записанными
                ...
                if (isnew) { // Если новое значение действительно новое, то...
                  ...
                  cnt_PDMcount = PDMset_SIZE - 1;       //Ставим счетчик на запомненную уставку
                }
...
//  uint32_t sqr = U_adc;
//  sqr *= U_adc; // Возводим в квадрат выборку АЦП
//  sum += sqr;   // Суммирование квадратов выборок АЦП
...
//  sqr *= U_adc; // Возводим в квадрат выборку АЦП
//  sum += sqr;   // Суммирование квадратов выборок АЦП
////  sum += U_adc * U_adc; // Возводим в квадрат выборку АЦП и суммируем с предыдущими
//  ++sc;     // Счетчик выборок АЦП
//  }//===Суммирование квадратов=======================================
  //
    ...
    switch (cnt_dspMenu) { //=====Проверяем режимы меню
      ...
      case 1:  {  //=============Если мы в меню выбора уставки, то...
        ...
        if ((PDMold != PDMset[0][cnt_PDMcount]) || fl.dspRefresh) {
          ...
          uint32_t p = calc_PDMtoPust(PDMold); // Считаем уставку с округлением
#ifdef INTERFACE_ALT
          ASOled.printString_12x16(F("    "), X_position (3,0,12), 0);
