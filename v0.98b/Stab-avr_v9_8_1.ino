...
#define CICLE 500         // Количество полупериодов в полном цикле регулирования (200 полупериодов - 2сек, если устанавливать более 255 - надо менять размерность сопутствующих переменных )
//
...
uint16_t *Pnom_arr = new uint16_t [Pnom_ARRAY_SIZE];  // Массив мощностей ТЭНа
...
#define INIT_KEY 248              // Значение в последней ячейке EEPROM после инициализации (не более 255)
...
const uint8_t cell_size = 2;      // Размер в байтах ячейки памяти для записи уставки
...
static uint16_t U_real = U_LINE_Q;        // Среднеквадратичное за секунду (целая часть)
...
//
#define SET_Pust {uint32_t tmp = Pnom * 2; tmp *= PDMust; tmp /= CICLE;  tmp++; Pust = tmp/2;}  // Считаем Pust с округлением
...
//
void EEPROM_init(void) {  //===========Подпрограмма начальной инициализации EEPROM================
  //
    if (EEPROM[EEPROM.length() - 1] != INIT_KEY){ // Если в последнюю ячейку не записан ключ инициализации
      for (unsigned int i = 2 * Pnom_ARRAY_SIZE; i < EEPROM.length() - 1;i += cell_size) {
        EEPROM.put(i,(uint16_t)EMPTY_CELL_VALUE);
      }
      EEPROM.put(EEPROM.length() - 1,(uint8_t)INIT_KEY);
    }
} //===========Подпрограмма начальной инициализации EEPROM================
//
void EEPROM_read_Pnoms(void) { //===========Подпрограмма чтения начальных данных из EEPROM================
//
  for (cnt.Pnom_count = 0; cnt.Pnom_count < Pnom_ARRAY_SIZE; cnt.Pnom_count++) {
    ...
    EEPROM.get((cnt.Pnom_count * 2),Pnom_arr[cnt.Pnom_count]);
//    cnt.Pnom_count = x;
    if (Pnom_arr[cnt.Pnom_count] == EMPTY_CELL_VALUE) { // Дошли до пустой ячейки
      break;
    }
  }
...
//
  uint16_t PDMtmp[2][PDMset_SIZE] = {};
  uint16_t val;
  uint8_t tmpcount = 0;
  uint16_t addr;
  addr = (EEPROM.length() - SRVDATA_ARRAY_SIZE - 2 * Pnom_ARRAY_SIZE)/(Pnom_ARRAY_SIZE);  // Размер области для записи уставок в EEPROM
  addr /= cell_size; addr *= cell_size;             // Округлим до cell_size в меньшую сторону
  start_addr = Pnom_ARRAY_SIZE * 2 + cnt.Pnom_number * addr;  // Начальный адрес области
  end_addr = start_addr + addr - cell_size;             // Конечный адрес области
  //
  ...
  EEPROM.get(end_addr,val);                 // Прочтем последнюю ячейку области
  if (val != EMPTY_CELL_VALUE) {                                      // Если считанное значение не пустое, то
    read_region_end_addr = start_addr + cell_size * ((PDMset_SIZE - 2) - 1);        // На 3 меньше, потому что одно значение уже есть, как минимум, другое в память не пишется, ну и размер на единицу больше наибольшего индекса, Так-то!
    for (addr = end_addr - cell_size * (PDMset_SIZE-2); addr <= end_addr; addr += cell_size) {  // Почитаем из области для записи уставок в EEPROM, начиная с конца
      EEPROM.get(addr,val);
      if (val != EMPTY_CELL_VALUE) {          // Если считанное значение не пустое, то
        PDMtmp[0][tmpcount] = val;              // запишем его в массив уставок
        PDMtmp[1][tmpcount] = addr;             // и туда же запишем адрес значения в памяти EEPROM
  ...
  if (tmpcount < PDMset_SIZE - 1) {                               // Проверим массив уставок на заполненность и если не полон, то
    for (addr = start_addr; addr <= read_region_end_addr; addr += cell_size) {  // Почитаем ещё из области для записи уставок в EEPROM, начиная с начала
      EEPROM.get(addr,val);
      if (val != EMPTY_CELL_VALUE) {                          // Если считанное значение не пустое, то
        PDMtmp[0][tmpcount] = val;                                // запишем его в массив уставок
        PDMtmp[1][tmpcount] = addr;                               // и туда же запишем адрес значения в памяти EEPROM
  ...
  if (tmpcount) { // Если записанные уставки есть, то...
    ...
    { // Уберем дублирующиеся значения
      ...
      for (int i = 0; i < PDMset_SIZE; i++) {   // Пробежимся по обоим массивам
        for (int j = 0; j <= tmpcount; j++) {
          if (PDMset[0][i] == PDMtmp[0][j]) ++PDMdiff[i]; // И заполним массив совпадений
        }
      }
      for (int i = 0; i < PDMset_SIZE - 1; i++) { // Сортируем
        for (int j = i + 1; j < PDMset_SIZE; j++) {
          if (PDMdiff[j]) {             // Если в данной позиции есть совпадение
            uint16_t t = PDMset[0][i];
            PDMset[0][i] = PDMset[0][j];// Обмениваемся
            PDMset[0][j] = t;
            PDMdiff[j] = PDMdiff[i];
            break;
          }
    ...
    for (int i = 0; i <= tmpcount; i++) { // Допишем в рабочий массив считанное из EEPROM
...
void PDM_(void) { //===========Подпрограмма управления твердотельным реле ТЭНа================
  ...
  static int32_t err = 0;   // Ошибка дискретизации
  static int8_t ps = 0;     // Текущее значение постоянной составляющей
  int32_t lev = pdm + err;    // Текущий уровень с учетом ошибки дискретизации, сделанной на предыдущем полупериоде.
      ...
      case 2:  {  //=============Если мы в начальном меню выбора номинальной мощности, то...
        if (fl.dspTimeout) {                  // Если кнопки слишком долго не нажимались...
          if (Pnom_arr[0] != 0xffff) {        // и есть записанное значение, уходим
            ...
            delete [] Pnom_arr;         // Массив запомненных номинальных мощностей больше не нужен
        ...
        switch (bt.butt) {
          ...
          case 8: { //-----Кнопкой "Разгон" выходим из менюшки
            if (Pnom < 10000) { // Если значение реальное...
              ...
              EEPROM.put((cnt.Pnom_number * 2),Pnom);
          ...
          case 4: { //=====По кнопке "стоп" записываем уставку, если нужно, принимаем и выходим
            ...
            if (fl.writable) {  // Если уставки запоминаются...
              if (!PDMset[1][cnt.PDMcount]) { // Если просят записать НЕ уже записанное...
                ...
                EEPROM.put(new_addr,PDMset[0][cnt.PDMcount]); // Пишем новую уставку
...
ISR(ADC_vect) { //===============Обработчик окончания преобразования АЦП===================
  ...
  static int16_t Uold = 0;           // Буферная переменная для НЧ-фильтрации
  static int16_t Udelta = 0;         // Буферная переменная для НЧ-фильтрации
  //
  Udelta += (U - Uold);   //
  U = Udelta / 32;      //КИХ ФНЧ 1-го порядка с коэффициентом 1/32
  ...
  //===Проверка насуммированных отсчетов============================
    ...
    register int32_t temp_32 = (TM2_current + PHASE) << Km; // Разность фаз
    if (!fl.PVolna) {
 ...
 //
  ...
  uint32_t sqr = U_adc * U_adc; // Возводим в квадрат выборку АЦП
...
//
  ...
  #endif
  //
  EEPROM_init();  // TODO! Инициализируем EEPROM
  //
  ASOled.clearDisplay();  // Очищаем, иначе некорректно работает для дисплеев на SH1106 (косяк библиотеки)
  ...
  //
  //Pnom = eeprom_read_word((uint16_t*)0);
  ...
  if (fl.DisplayOut) { //========================Вывод информации на дисплей=============
    switch (fl.dspMenu) { //=====Проверяем режимы меню
      case 2:  {  //=============Если мы в начальном меню, то...
        if (fl.dspRefresh) {  //Обновляем дисплей
          ...
          ASOled.printString_6x8(F("Pном="), 0, 1);
          ASOled.printString_6x8(F("Вт"), X_position (14), 1);
#else
          ASOled.printString_6x8(F("Выберите/введите Pном"), 0, 0);
          ASOled.printString_6x8(F("Pном=      Вт, (   В)"), 0, 1);
      ...
      case 1:  {  //=============Если мы в меню выбора уставки, то...
        if (fl.dspRefresh) {  //Обновляем дисплей
          ASOled.clearDisplay();
#ifdef INTERFACE_ALT
          ASOled.printString_6x8(F("Pуст="), 0, 1);
          ASOled.printString_6x8(F("Вт"), X_position (14), 1);
#else
        ...
        if ((PDMold != PDMset[0][cnt.PDMcount]) || fl.dspRefresh) {
          ...
          uint32_t p = Pnom * 2; p *= PDMold; p /= CICLE;  p++; p /= 2; // Считаем уставку с округлением
#ifdef INTERFACE_ALT
          ASOled.printString_12x16(F("    "), X_position (3,0,12), 0);
    ...
    #define str_Ureal 0
        ...
        if ((PDMust_old != PDMust) || fl.dspRefresh) {
          PDMust_old = PDMust;
          uint16_t x = 100*PDMust_old;
          uint8_t percent = x / CICLE;    // посчитаем процент
          x -= percent * CICLE;
          x *= 10;
          uint8_t percent_dec = x / CICLE;  // посчитаем десятые процента
#ifdef INTERFACE_ALT
