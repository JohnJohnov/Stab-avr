...
//--добавлено общение по USART с управляющей программой
//--
//-------------------------
#include <Wire.h>
#include <EEPROM.h>
...
#include <ASOLED.h>
//
#define VERSION "v0.96"   // Версия скетча
...
#define USART_ON          // Раскомментить для включения общения с внешним контроллером
//========================================
//
...
static uint8_t Pcount_tm = 0;   // Счетчик полупериодов для организации отсчета времени
...
//
#define U_LINE 230          // Номинальное значение действующего напряжения в сети
...
static volatile struct flags {
  ...
  unsigned  report : 1;       // Флаг разрешения отправки данных внешнему контроллеру
} fl = {0};
//
//
...
//
// Функция переводит символ ASCII в шестнадцатиричную цифру, при ошибке возвращает 16
...
//
  while (Serial.available() > 0) {
    ...
    USART_SetData[0] = Serial.read(); // Вычитываем очередной байт
      if ((USART_SetData[0] == 0x4D) || (USART_SetData[0] == 0x6D)) { // Ждём первый символ запроса на изменение режима работы
        data_size = 2;
        ++index;
      }
      else if ((USART_SetData[0] == 0x50) || (USART_SetData[0] == 0x70)) { // Ждём первый символ запроса на изменение уставки
        data_size = 5; 
        ++index;       
      }
    }
      ...
      if ( USART_SetData[index] == 0x0D ) { // Ждем последнего символа посылки
        if ( index == data_size ) {
          switch (index) {
            case 2: { // Парсим запрос на смену режима
              switch ( USART_SetData[1] ) {
                ...
                case '1': { // Переход в режим разгона
                  if ((!fl.NotZero) & (!fl.Udown)) {  // Если сеть в дауне - не разгонишься
                      fl.razg_on = 1;
                      fl.razg = 1;
                  } 
                  break;
            ...
            case 5: { // Парсим запрос на смену уставки
              ...
              else if (tmp_p >= Pnom) {  // Проверяем присланную уставку на не превышение номинальной мощности
                PDMust = CICLE;
              }
              else {
                tmp_p *= CICLE;
...
//
void USART_report() { //=====Отчет внешнему контроллеру=====
  uint16_t b;
  //
  ...
  if (fl.razg_on || fl.Ulow) { // В разгоне и при недостаточном сетевом передаем расчетную текущую мощность
        ...
        switch (bt.butt) {
          ...
          case 8: { //-----Кнопкой "Разгон" выходим из менюшки
            if (Pnom < 10000) { // Если значение реальное...
              ...
              #ifdef USART_ON   //========================================
                USART_start();  // Вот теперь можно инициализировать USART
              #endif            //========================================
...
//
  ...
  fl.PP = 1;
  //
  if (++Pcount_tm == P_TIME_MAX) {  // Уже секунду суммируем
    #ifdef USART_ON
      fl.report = 1;                  // пора слать рапорт
    #endif
    //
    Pcount_tm = 0;
  }
...
//    if (PVolna == (-1)) {
    ...
    // Готовим данные для записи в регистр сравнения таймера 2
      ...
      //
      Pcount_tm = 0;    // сбросим счетчик полупериодов
    }
 //
...
// 
  ...
  //
  #ifdef USART_ON
  if (fl.report) {  //==========Отправка отчета внешнему контроллеру============
    USART_report();
    fl.report = 0;
